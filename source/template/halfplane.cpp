...
struct Halfplane { 
    ...
    // Be careful when handling parallel cases.
...
FOR(j, 0, n - 1){
    cin >> pts[j].x >> pts[j].y;
    if (j) H.pb(Halfplane(pts[j - 1], pts[j]));
} 
H.pb(Halfplane(pts.back(), pts[0]));
vector<Point> CH = hp_intersect(H);